    ...
    <FieldGroup className="gap-6">
      ...
      <div className="inline-flex rounded-lg border border-border bg-muted/40 p-1">
        <button
          ...
          onClick={() => { onReceiverModeChange("existing"); onMapPreviewChange(null); }}
        >
          {p("existingCustomer")}
        </button>
        <button
          ...
          onClick={() => { onReceiverModeChange("new"); onMapPreviewChange(null); }}
        >
          {p("newProfile")}
        </button>
      </div>
      ...
      {receiverMode === "existing" ? (
        <Field data-invalid={!!errors.receiverId}>
          <FieldLabel htmlFor="receiver-customer">{p("editCardReceiver")}</FieldLabel>
          <Controller
            control={control}
            name="receiverId"
            render={({ field }) => (
          ...
          />
          {errors.receiverId && <FieldError>{errors.receiverId.message}</FieldError>}
        ...
        <div className="grid grid-cols-1 gap-4 rounded-xl border border-border bg-muted/20 p-4 md:grid-cols-2 md:p-6">
          <Field data-invalid={!!errors.receiver?.name}>
            ...
            <Input id="receiver-name" placeholder={sw("placeholderNameReceiver")} aria-invalid={!!errors.receiver?.name} {...register("receiver.name")} />
            {errors.receiver?.name && <FieldError>{errors.receiver.name.message}</FieldError>}
          </Field>

          <Field data-invalid={!!errors.receiver?.email}>
            <FieldLabel htmlFor="receiver-email">{fc("email")}</FieldLabel>
            <Input id="receiver-email" type="email" placeholder={tl("emailPlaceholder")} aria-invalid={!!errors.receiver?.email} {...register("receiver.email")} />
            {errors.receiver?.email && <FieldError>{errors.receiver.email.message}</FieldError>}
          </Field>

          <Field data-invalid={!!errors.receiver?.phone}>
            <FieldLabel htmlFor="receiver-phone">{p("phone")}</FieldLabel>
            <Input id="receiver-phone" placeholder="+237 …" aria-invalid={!!errors.receiver?.phone} {...register("receiver.phone")} />
            {errors.receiver?.phone && <FieldError>{errors.receiver.phone.message}</FieldError>}
          </Field>

          <Field className="md:col-span-2" data-invalid={!!errors.receiver?.address}>
            <FieldLabel htmlFor="receiver-address-search">{p("addressSearch")}</FieldLabel>
            <AddressAutocomplete
            ...
            {errors.receiver?.address && <FieldError>{errors.receiver.address.message}</FieldError>}
            {mapPreview && (
          ...
          <Field data-invalid={!!errors.receiver?.city}>
            ...
            <Input id="receiver-city" placeholder={ad("autofilled")} aria-invalid={!!errors.receiver?.city} {...register("receiver.city")} />
            {errors.receiver?.city && <FieldError>{errors.receiver.city.message}</FieldError>}
          </Field>

          <Field data-invalid={!!errors.receiver?.state}>
            <FieldLabel htmlFor="receiver-state">{p("state")}</FieldLabel>
            <Input id="receiver-state" placeholder={ad("statePlaceholder")} aria-invalid={!!errors.receiver?.state} {...register("receiver.state")} />
            {errors.receiver?.state && <FieldError>{errors.receiver.state.message}</FieldError>}
          </Field>

          <Field data-invalid={!!errors.receiver?.zipCode}>
            <FieldLabel htmlFor="receiver-zip">{p("zipCode")}</FieldLabel>
            <Input id="receiver-zip" placeholder={ad("zipPlaceholder")} aria-invalid={!!errors.receiver?.zipCode} {...register("receiver.zipCode")} />
            {errors.receiver?.zipCode && <FieldError>{errors.receiver.zipCode.message}</FieldError>}
          </Field>

          <Field data-invalid={!!errors.receiver?.country}>
            <FieldLabel htmlFor="receiver-country">{p("country")}</FieldLabel>
            <Input id="receiver-country" placeholder={ad("countryPlaceholder")} aria-invalid={!!errors.receiver?.country} {...register("receiver.country")} />
            {errors.receiver?.country && <FieldError>{errors.receiver.country.message}</FieldError>}
          </Field>
        </div>
      )}
    </FieldGroup>
